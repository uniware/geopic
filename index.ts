...
import { indexImpl, GeoData, asClusters, GeoItem, boundsForDateRange, Filter, numToDate } from './geoitem.js';
...
let g_geoData: GeoData | undefined;
...
let g_filter: Filter = { dateRange: undefined, text: undefined };
...
export async function onBodyLoad(): Promise<void> {
    ...
    if (code || state) window.history.replaceState(null, '', window.location.pathname);

    MAP = (document.getElementById("map")! as google.maps.MapElement).innerMap;
    MARKER_POOL = new MarkerPool(MAP, await google.maps.importLibrary("marker") as google.maps.MarkerLibrary);
    ...
    g_geoData = await dbGet<GeoData>();
    if (g_geoData && g_geoData.id === 'sample-data' && (code || localStorage.getItem('access_token'))) g_geoData = undefined;
 ...
 */
async function installHandlers(): Promise<void> {
    let userHasMapWork = false;
    let boundsChangedByCode = false;

    MAP.addListener('bounds_changed', () => {
        if (!g_geoData) return;
        if (!boundsChangedByCode) userHasMapWork = Boolean(g_filter.dateRange);
        showCurrentGeodata();
    });
    ...
    OVERLAY.siblingGetter = (id, direction) => {
        if (!g_geoData || g_geoData.id === 'sample-data') return undefined;
        ...
        else if (direction === 'prev') return index <= 0 ? undefined : items[index - 1];
...
function instruct(mode: 'indexing' | 'fresh' | 'stale' | undefined): void {
 ...
 */
export async function onLogoutClick(): Promise<void> {
    await dbPut(null);
 ...
 */
function showCurrentGeodata(): void {
    ...
    function showItem(item: GeoItem): void {
        if (g_geoData?.id === 'sample-data') OVERLAY.showDataUrl(item.id, item.thumbnailUrl, `<h2>HIGH QUALITY ONLY FOR YOUR OWN ONEDRIVE PHOTOS</h2> ${escapeHtml(item.name)}`);
 ...
 */
export async function onIndexClick(): Promise<void> {
    instruct('indexing');
