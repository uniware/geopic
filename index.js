...
async function installHandlers() {
    ...
    async function selectPlaceSuggestion(prediction) {
    ...
    function onPlaceSuggestionsKeydown(e) {
    ...
    function onFilterTextChangedForPlaceSuggestions(value) {
        ...
        debounceTimer = setTimeout(async () => {
            ...
            if (TEXT_FILTER.value.trim() !== text)
                return; // stale response - the filter text has since changed again
            renderPlaceSuggestions(suggestions.map(s => s.placePrediction).filter((p) => p !== null));
        }, 250);
    }
    ...
    document.addEventListener('click', (e) => {
        if (!SUGGESTIONS.contains(e.target) && e.target !== TEXT_FILTER)
            hidePlaceSuggestions();
    });
