...
export async function exchangeCodeForToken(CLIENT_ID: string, code: string, code_verifier: string): Promise<{ accessToken: string, refreshToken: string } | FetchError> {
    ...
    console.log('Code verifier present:', code_verifier ? 'yes' : 'no');
    ...
    if (!response.ok) {
        ...
        return new FetchError(`${url}[exchangeCodeForToken]`, response, errorText);
    }

    const tokenData = await response.json();
    console.log('Token exchange successful. Received access_token:', tokenData.access_token ? 'yes' : 'no');
    console.log('Received refresh_token:', tokenData.refresh_token ? 'yes' : 'no');
